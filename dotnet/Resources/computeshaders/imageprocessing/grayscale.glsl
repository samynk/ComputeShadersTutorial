...
void main() {
	// ivec2 : 2D vector (xy but also rg and also uv)
	ivec2 gid = ivec2(gl_GlobalInvocationID.xy); // swizzle
	// imageLoad gives me a vec4
    vec3 color = imageLoad( inputImage, gid ).rgb; // can use rgb
	float gray = 0.299*color.r + 0.587*color.g + 0.114*color.b;
	// vec4(1,0.2,0.3,0);
	// imageStore(outputImage, gid, vec4( color.brg, 1));
